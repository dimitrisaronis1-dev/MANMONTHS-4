...
# Try to load the template file directly from the repository
TEMPLATE_FILE_NAME_IN_REPO = "AM TEST 1.xlsx" # Adjust this name if your template file has a different name
template_file_buffer = None
try:
    with open(TEMPLATE_FILE_NAME_IN_REPO, "rb") as f:
        template_file_buffer = io.BytesIO(f.read())
    st.info(f"Το αρχείο template '{TEMPLATE_FILE_NAME_IN_REPO}' φορτώθηκε αυτόματα από το repository.")
except FileNotFoundError:
    st.warning(f"Το αρχείο template '{TEMPLATE_FILE_NAME_IN_REPO}' δεν βρέθηκε στο repository. Παρακαλώ ανεβάστε το.")
    # Fallback to file uploader if not found in repo
    uploaded_template = st.file_uploader("Ανεβάστε το αρχείο template (Excel)", type=["xlsx"], key="template_upload")
    if uploaded_template:
        template_file_buffer = io.BytesIO(uploaded_template.read())

input_file = st.file_uploader("Ανεβάστε το αρχείο input (Excel)", type=["xlsx"], key="input_upload")

# Check if both files are available (either from repo or uploaded)
if template_file_buffer is not None and input_file is not None:
    st.success("Και τα δύο αρχεία είναι διαθέσιμα!")

    # Process the files
    if st.button("Εκτέλεση Κατανομής"):
        try:
            with st.spinner('Επεξεργασία δεδομένων...'):
                # Pass both file buffers to the processing function
                output_excel_buffer, yearly_am_totals, unallocated_projects, yearly_overages, MAX_YEARLY_CAPACITY = process_excel_data(input_file, template_file_buffer)
